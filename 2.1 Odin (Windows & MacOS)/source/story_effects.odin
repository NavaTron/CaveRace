...
// draw_branding_effects animates only features already present in the
// current 800x480 cave-mouth artwork: the mine entrance lantern, the wall
// torch, a wisp of volcano smoke, red energy on the "N" mark, metallic
// title highlights, the mine cart's and crate's gem glints, the two crystal
// clusters, and a few slow dust motes drifting near the dark rock frame.
draw_branding_effects :: proc(
	...
	draw_flame_effect(
		...
		16,
		clock + 0.45,
		transition_alpha * 0.52,
		reduced_flashes,
	)
	draw_story_flame({756, 172}, clock, transition_alpha, reduced_flashes)
	draw_flame_effect(
		effects.flame,
		755,
		180,
	...
	draw_story_light(
		...
		122,
		rl.RED,
		0.19 + mark_pulse * 0.13,
		transition_alpha,
		reduced_flashes,
	)
	mark_points := [6]Story_Point {
		{344, 61}, {461, 63}, {309, 105},
		{410, 97}, {359, 184}, {466, 171},
	}
	...
	wordmark_points := [5]Story_Point {
		{214, 228}, {291, 253}, {367, 237}, {438, 221}, {566, 256},
	}
	...
	treasure := [7]Story_Point {
		{55, 281}, {78, 273}, {99, 267}, {113, 284}, {137, 282},
		{651, 374}, {676, 366},
	}
	...
	crystals := [6]Story_Point {
		{39, 374}, {18, 407}, {72, 422},
		{724, 311}, {687, 350}, {762, 367},
	}
	...
	dust := [7]Story_Point {
		{35, 90}, {745, 85}, {40, 220}, {760, 220},
		{55, 430}, {735, 410}, {400, 35},
	}
...
draw_story_effects :: proc(
	...
	case .Stars:
		...
		draw_story_light({585, 250}, 136, rl.SKYBLUE, 0.26 + planet_pulse * 0.14, transition_alpha, reduced_flashes)
		draw_story_light({659, 62}, 52, rl.MAGENTA, 0.22, transition_alpha, reduced_flashes)
		points := [16]Story_Point {
			{504, 56}, {450, 75}, {500, 20}, {533, 91},
			{570, 69}, {707, 17}, {765, 59}, {789, 145},
			{421, 464}, {480, 449}, {530, 429}, {580, 457},
			{635, 434}, {689, 458}, {747, 420}, {786, 368},
		}
	...
	case .Mining_Glints:
		lanterns := [4]Story_Point {{620, 139}, {676, 115}, {92, 412}, {574, 246}}
		...
		draw_flame_effect(effects.flame, 620, 160, 11, 22, clock, transition_alpha * 0.58, reduced_flashes)
		draw_flame_effect(effects.flame, 676, 137, 10, 21, clock + 0.6, transition_alpha * 0.56, reduced_flashes)
		draw_flame_effect(effects.flame, 92, 430, 12, 24, clock + 1.1, transition_alpha * 0.60, reduced_flashes)
		gold := [8]Story_Point {
			{197, 308}, {224, 295}, {250, 320}, {276, 329},
			{304, 301}, {326, 307}, {357, 316}, {389, 306},
		}
		...
		crystals := [8]Story_Point {
			{743, 218}, {735, 244}, {754, 252}, {585, 422},
			{603, 411}, {675, 399}, {707, 367}, {744, 392},
		}
		crystal_count := story_effect_count(len(crystals), reduced_flashes)
		for crystal, crystal_index in crystals[:crystal_count] {
			pulse := story_effect_pulse(clock, crystal_index * 6 + 3, 26, reduced_flashes)
			color := rl.SKYBLUE
			if crystal_index >= 5 do color = rl.MAGENTA
			draw_story_glint(crystal, 3, color, pulse, transition_alpha, reduced_flashes)
		}
	...
	case .Alien_Eyes:
		draw_ambient_smoke(
			...
			clock + 0.9,
			46,
			rl.Color{116, 72, 148, 255},
			transition_alpha * 0.10,
			reduced_flashes,
		)
		eyes := [3]Story_Point {{415, 250}, {595, 111}, {635, 335}}
	...
	case .TNT_Fuse:
		...
		draw_flame_effect(effects.flame, 92, 344, 12, 24, clock, transition_alpha * 0.58, reduced_flashes)
		draw_flame_effect(effects.flame, 571, 239, 11, 22, clock + 0.7, transition_alpha * 0.56, reduced_flashes)
		draw_story_fuse({472, 252}, clock, 0, transition_alpha, reduced_flashes)
		draw_story_fuse({515, 352}, clock, 9, transition_alpha, reduced_flashes)

	case .Torch_And_Treasure:
		draw_story_flame({262, 291}, clock, transition_alpha, reduced_flashes)
		draw_flame_effect(
			effects.flame,
			258,
			329,
			...
			66,
			clock,
			transition_alpha * 0.74,
			reduced_flashes,
			11,
		)
		helmet_lights := [2]Story_Point {{213, 284}, {583, 290}}
		for helmet_light, light_index in helmet_lights {
			helmet_pulse := story_effect_pulse(clock, 9 + light_index * 7, 25, reduced_flashes)
			draw_story_light(helmet_light, 17, rl.GOLD, 0.62 + helmet_pulse * 0.24, transition_alpha, reduced_flashes)
		}
		gold := [8]Story_Point {
			{383, 369}, {414, 348}, {447, 335}, {478, 344},
			{519, 329}, {555, 352}, {590, 369}, {500, 382},
		}
		count := story_effect_count(len(gold), reduced_flashes)
		for point, point_index in gold[:count] {
		...
		draw_story_glint({478, 395}, 5, rl.SKYBLUE, diamond_pulse, transition_alpha, reduced_flashes)
		potion_pulse := story_effect_pulse(clock, 13, 24, reduced_flashes)
		draw_story_glint({711, 381}, 4, rl.SKYBLUE, potion_pulse, transition_alpha, reduced_flashes)

	case .Explosion:
		blast_pulse := story_effect_pulse(clock, 0, 20, reduced_flashes)
		lantern_pulse := story_effect_pulse(clock, 8, 27, reduced_flashes)
		draw_story_light({92, 351}, 19, rl.GOLD, 0.64 + lantern_pulse * 0.24, transition_alpha, reduced_flashes)
		draw_flame_effect(effects.flame, 92, 370, 10, 20, clock + 0.4, transition_alpha * 0.56, reduced_flashes)
		blast_center := Story_Point{608, 250}
		draw_story_light(blast_center, 86, rl.ORANGE, 0.62 + blast_pulse * 0.26, transition_alpha, reduced_flashes)
		draw_story_light(blast_center, 35, rl.GOLD, 0.76 + blast_pulse * 0.18, transition_alpha, reduced_flashes)
		draw_story_explosion(
			effects,
			blast_center,
			clock,
			154,
			transition_alpha * 0.58,
			reduced_flashes,
		)
		rays := [6]Story_Point {{560, 153}, {649, 158}, {522, 218}, {696, 245}, {557, 317}, {662, 325}}
		...
		for ray in rays[:ray_count] {
			...
			rl.DrawLine(blast_center.x, blast_center.y, ray.x, ray.y, rl.Fade(rl.GOLD, ray_alpha))
		}
		smoke_origin := Story_Point {608, 201}
		...
		ember_origin := Story_Point {608, 253}
		...
		draw_story_fuse({293, 313}, clock, 5, transition_alpha, reduced_flashes)
		diamond_pulse := story_effect_pulse(clock, 3, 24, reduced_flashes)
		draw_story_glint({514, 416}, 5, rl.SKYBLUE, diamond_pulse, transition_alpha, reduced_flashes)
		potion_pulse := story_effect_pulse(clock, 12, 24, reduced_flashes)
		draw_story_glint({650, 414}, 4, rl.GREEN, potion_pulse, transition_alpha, reduced_flashes)

	case .Homecoming:
		exit_pulse := story_effect_pulse(clock, 5, 34, reduced_flashes)
		draw_story_light({650, 148}, 118, rl.SKYBLUE, 0.16 + exit_pulse * 0.11, transition_alpha, reduced_flashes)
		lantern_pulse := story_effect_pulse(clock, 9, 27, reduced_flashes)
		draw_story_light({508, 189}, 23, rl.GOLD, 0.66 + lantern_pulse * 0.24, transition_alpha, reduced_flashes)
		draw_flame_effect(effects.flame, 508, 207, 11, 23, clock + 0.5, transition_alpha * 0.58, reduced_flashes)
		draw_ambient_smoke(
			effects,
			{650, 211},
			clock + 1.3,
		...
		gold := [8]Story_Point {
			{461, 285}, {489, 272}, {516, 294}, {543, 278},
			{570, 301}, {499, 309}, {533, 298}, {595, 286},
		}
		count := story_effect_count(len(gold), reduced_flashes)
		for point, point_index in gold[:count] {
		...
		motes := [8]Story_Point {
			{576, 84}, {615, 67}, {653, 91}, {693, 66},
			{733, 102}, {600, 147}, {706, 155}, {753, 207},
		}

...
begin_gameplay_action_interval :: proc(
	...
	action := select_gameplay_action(&gameplay.tick_state.input)
...
finish_gameplay_action_interval :: proc(
	...
	if pickup.treasure_collected {
		result.treasures_collected += 1
		gameplay.treasure_collected += 1
	}
...
run_gameplay_ticks :: proc(
	...
	tick_state.accumulator_seconds += clamp(frame_seconds, 0, MAX_FRAME_DELTA_SECONDS)

	for tick_state.accumulator_seconds + 1e-12 >= GAMEPLAY_TICK_SECONDS &&
	    ticks_run < MAX_GAMEPLAY_TICKS_PER_FRAME {
		...
		apply_queued_cheats(gameplay, cheats_enabled)
		...
		if tick_state.action_step == PLAYER_REVERSAL_STEP {
			try_reverse_player_at_halfway(gameplay)
		}

...
apply_explosion_to_level :: proc(
	...
	for cell_index in 0 ..< explosion.cell_count {
		...
		destroyed := treasure^ != 0 || (item^ != 0 && item^ < INDESTRUCTIBLE_ITEM_FIRST)
		...
		if destroyed {
			assert(result.destructibles_destroyed < MAX_DESTRUCTION_EVENTS)
			result.destructible_positions[result.destructibles_destroyed] = cell.position
			result.destructibles_destroyed += 1
		}
	}
}
...
start_ready_explosions :: proc(
	...
	for {
		...
		for bomb_index in 0 ..< MAX_BOMBS {
			...
			apply_explosion_to_level(gameplay, explosion, result)
...
// explosion_animation_set expands through the master's three explosion rows,
// then contracts through them before the fixed blast lifetime ends.
...
// explosion_sprite_index combines animation stage and the master sheet's
// center/up/right/down/left column order to locate the converted strip sprite.

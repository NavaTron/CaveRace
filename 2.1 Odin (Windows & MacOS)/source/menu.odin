...
update_menu :: proc(
	...
	if menu.page == .Settings && (input.pointer.primary_pressed || input.pointer.wheel != 0) {
		if hovered, ok := menu_hovered_item(menu, input.pointer); ok {
			...
			if item != .Bindings && item != .Back {
				...
				if delta != 0 {
					result.settings_changed, result.display_changed =
						adjust_setting(settings, item, delta)
					return result
				}
			}
		}
	...
	if menu.page == .Settings && (input.menu_left_pressed || input.menu_right_pressed) {
		...
		result.settings_changed, result.display_changed = adjust_setting(
			settings,
	...
	case .Settings:
		...
		if item == .Bindings {
			...
			result.settings_changed, result.display_changed =
				adjust_setting(settings, item, 1)
		}
	case .Bindings:

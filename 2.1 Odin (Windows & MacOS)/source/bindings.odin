...
controller_actions_may_share :: proc(first, second: Input_Action) -> bool {
	return (first == .Bomb && second == .Confirm) ||
	       (first == .Confirm && second == .Bomb)
}

// Controller A may be shared by Bomb and Confirm because those actions are
// used in different contexts. All other controller bindings stay unique.
controller_bindings_are_valid :: proc(bindings: Controller_Bindings) -> bool {
	for button, action in bindings {
		...
		for other_index in int(action) + 1 ..< len(Input_Action) {
			...
			if controller_actions_may_share(action, other) do continue
			return false
		}
	}
	return true
...
try_rebind_controller_action :: proc(
	...
	for existing, existing_action in bindings {
		...
		if controller_actions_may_share(action, existing_action) do continue
		return false
	}
...
// action_prompt returns the current device's label for an action.
action_prompt :: proc(
	action: Input_Action,
	device: Input_Device,
	bindings: Keyboard_Bindings,
	...
	if device == .Controller {
		return controller_button_label(controller_bindings[action])
	}

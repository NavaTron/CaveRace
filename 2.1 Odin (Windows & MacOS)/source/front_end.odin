...
// Keep this development switch available for quickly reaching the menu while
// iterating, but ship with the complete story sequence enabled after Branding.
// The main menu's "Replay Story" item also remains available on demand.
SKIP_STORY_INTRO :: false
...
advance_intro :: proc(
	...
	assert(front_end.image_index >= INTRO_FIRST_IMAGE && front_end.image_index <= INTRO_LAST_IMAGE)
	durations := INTRO_IMAGE_SECONDS
	image_seconds := durations[front_end.image_index - INTRO_FIRST_IMAGE]
	if !music_finished {

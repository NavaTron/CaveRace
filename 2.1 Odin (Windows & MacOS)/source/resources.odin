...
// media_resource_path_cstring resolves a packaged media path in the format
// expected by raylib. The returned C string is owned by the caller.
media_resource_path_cstring :: proc(
	root, relative_path: string,
	allocator := context.allocator,
) -> (path: cstring, ok: bool) {
	resource, resource_ok := resource_path(
		root,
		{RESOURCE_MEDIA_DIRECTORY, relative_path},
		allocator,
	)
	if !resource_ok do return nil, false
	defer delete(resource, allocator)
	cloned_path, clone_error := strings.clone_to_cstring(resource, allocator)
	return cloned_path, clone_error == nil
}
...
resource_root_is_usable :: proc(root: string) -> bool {
	marker_path, ok := resource_path(
		root,
		{RESOURCE_MEDIA_DIRECTORY, "screens", "status_bar.png"},

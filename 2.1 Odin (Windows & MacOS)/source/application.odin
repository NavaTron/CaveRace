...
run_application :: proc(options: Launch_Options) -> bool {
	...
	init_game(&app.game, settings, options.cheats_enabled)
	...
	rl.SetTargetFPS(TARGET_RENDER_FPS)
	...
	for !rl.WindowShouldClose() && !app.quit_requested {
		...
		process_game_requests(&app, &update_result)
...
music_gain_for_game :: proc(game: ^Game) -> f32 {
	if game.pause.open do return 0.5
	return 1
}

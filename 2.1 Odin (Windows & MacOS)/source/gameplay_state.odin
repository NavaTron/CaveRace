...
EXPLOSION_STEPS     :: 12
MAX_DESTRUCTION_EVENTS :: MAX_BOMBS * (MAX_EXPLOSION_CELLS - 1)
...
PLAYER_DOWN_FIRST_SPRITE     :: 0
PLAYER_UP_FIRST_SPRITE       :: 6
PLAYER_LEFT_FIRST_SPRITE     :: 12
PLAYER_RIGHT_FIRST_SPRITE    :: 18
PLAYER_DIRECTION_FRAME_COUNT :: 6
PLAYER_ANIMATION_TICKS_PER_FRAME :: 6
...
#assert(MAX_DESTRUCTION_EVENTS == 160)
...
Gameplay_Tick_Result :: struct {
	...
	destructibles_destroyed: int,
	destructible_positions:  [MAX_DESTRUCTION_EVENTS]Grid_Position,
	squish_requests:         int,
	items_collected:         int,
	items_salvaged:          int,
...
// Explosion_Cell_Kind follows the bomb master sheet's column order so adding
// the enum value to a stage's first sprite selects the matching direction.
...
#assert(int(Explosion_Cell_Kind.Center) == 0)
#assert(int(Explosion_Cell_Kind.Up) == 1)
#assert(int(Explosion_Cell_Kind.Right) == 2)
#assert(int(Explosion_Cell_Kind.Down) == 3)
#assert(int(Explosion_Cell_Kind.Left) == 4)

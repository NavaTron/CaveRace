package caverace

import rl "vendor:raylib"

// Screen_Assets owns branding, gameplay/outcome backgrounds, and all
// front-end images for the complete application asset lifetime.
Screen_Assets :: struct {
	status_bar: rl.Texture,
	game_over:  rl.Texture,
	you_won:    rl.Texture,
	branding:   rl.Texture,
	front_end:  [FRONT_END_IMAGE_COUNT]rl.Texture,
}

// Music_Cue names every track used by the intro, menu, gameplay, and terminal screens.
Music_Cue :: enum {
	Branding,
	Intro_Eldora,
	Intro_Mining,
	Intro_Aliens,
	Intro_Defense,
	Intro_Hero,
	Intro_Bombs,
	Intro_Protect,
	Menu,
	Level_Complete,
	You_Won,
	Game_Over,
}

#assert(len(Music_Cue) == 12)
#assert(int(Music_Cue.Intro_Protect) - int(Music_Cue.Intro_Eldora) + 1 == INTRO_LAST_IMAGE - INTRO_FIRST_IMAGE + 1)

// Resource manifests keep array indices and enum values as the single mapping
// between domain-facing asset slots and packaged filenames.
FRONT_END_TEXTURE_PATHS :: [FRONT_END_IMAGE_COUNT]string {
	"intro/01_intro_eldora.png",
	"intro/02_intro_mining.png",
	"intro/03_intro_aliens.png",
	"intro/04_intro_defense.png",
	"intro/05_intro_hero.png",
	"intro/06_intro_bombs.png",
	"intro/07_intro_protect.png",
	"screens/menu.png",
	"screens/controls.png",
}

BOMB_SOUND_PATHS :: [BOMB_SOUND_COUNT]string {
	"sounds/bomb01.ogg",
	"sounds/bomb02.ogg",
	"sounds/bomb03.ogg",
	"sounds/bomb04.ogg",
}

MUSIC_PATHS :: [Music_Cue]string {
	.Branding       = "intro/00_branding.ogg",
	.Intro_Eldora   = "intro/01_intro_eldora.ogg",
	.Intro_Mining   = "intro/02_intro_mining.ogg",
	.Intro_Aliens   = "intro/03_intro_aliens.ogg",
	.Intro_Defense  = "intro/04_intro_defense.ogg",
	.Intro_Hero     = "intro/05_intro_hero.ogg",
	.Intro_Bombs    = "intro/06_intro_bombs.ogg",
	.Intro_Protect  = "intro/07_intro_protect.ogg",
	.Menu           = "music/menu.ogg",
	.Level_Complete = "music/level_complete.ogg",
	.You_Won        = "music/you_won.ogg",
	.Game_Over      = "music/game_over.ogg",
}

// Sound_Assets owns all raylib sound handles used when frame events request
// bomb, pickup, enemy, or ticking audio.
Sound_Assets :: struct {
	bomb:    [BOMB_SOUND_COUNT]rl.Sound,
	item:    rl.Sound,
	hit:     rl.Sound,
	squish:  rl.Sound,
	ticking: rl.Sound,
	menu:    rl.Sound,
	death:   rl.Sound,
}

// Sprite_Assets groups the grid-packed sprite sheets consumed by level, actor,
// explosion, and HUD rendering, plus the standalone level-complete
// celebration portraits (not tile sheets, so sized and validated separately).
Sprite_Assets :: struct {
	bomb:           rl.Texture,
	enemy:          rl.Texture,
	objects:        rl.Texture,
	player:         rl.Texture,
	tools:          rl.Texture,
	treasure:       rl.Texture,
	level_complete: [LEVEL_COMPLETE_SPRITE_COUNT]rl.Texture,
}

// Effect_Assets owns the optimized runtime atlases derived from the larger
// source VFX sheets in media/effects. These textures are presentation-only;
// gameplay collision and timing continue to use the legacy grid sprites.
Effect_Assets :: struct {
	explosion:       rl.Texture,
	explosion_smoke: rl.Texture,
	flame:           rl.Texture,
	smoke:           [2]rl.Texture,
}

LEVEL_COMPLETE_SPRITE_COUNT :: 3
LEVEL_COMPLETE_SPRITE_WIDTH :: 150
LEVEL_COMPLETE_SPRITE_HEIGHT :: 200

EFFECT_EXPLOSION_ATLAS_WIDTH  :: 480
EFFECT_EXPLOSION_ATLAS_HEIGHT :: 480
EFFECT_FLAME_ATLAS_WIDTH      :: 720
EFFECT_FLAME_ATLAS_HEIGHT     :: 384
EFFECT_SMOKE_TEXTURE_SIZE     :: 128

LEVEL_COMPLETE_SPRITE_PATHS :: [LEVEL_COMPLETE_SPRITE_COUNT]string {
	"sprites/level_complete_1.png",
	"sprites/level_complete_2.png",
	"sprites/level_complete_3.png",
}

// Assets is the application-owned aggregate loaded before the main loop and
// released as one unit during shutdown or failed startup.
Assets :: struct {
	screens: Screen_Assets,
	sounds:  Sound_Assets,
	music:   [Music_Cue]rl.Music,
	sprites: Sprite_Assets,
	effects: Effect_Assets,
	tiles:   [Tile_Theme]rl.Texture,
}

// load_resource_texture loads one media-relative texture during initialization.
load_resource_texture :: proc(root, relative_path: string) -> rl.Texture {
	path, ok := media_resource_path_cstring(root, relative_path)
	if !ok do return {}
	defer delete(path)
	return rl.LoadTexture(path)
}

// load_resource_sound resolves and loads one sound effect when the audio
// device is available during application startup.
load_resource_sound :: proc(root, relative_path: string) -> rl.Sound {
	path, ok := media_resource_path_cstring(root, relative_path)
	if !ok do return {}
	defer delete(path)
	return rl.LoadSound(path)
}

// load_resource_music resolves a streamed OGG track without decoding the
// complete song into memory during startup.
load_resource_music :: proc(root, relative_path: string) -> rl.Music {
	path, ok := media_resource_path_cstring(root, relative_path)
	if !ok do return {}
	defer delete(path)
	return rl.LoadMusicStream(path)
}

// load_assets fills the application-owned asset bundle and validates every
// required handle before the main loop is allowed to start. Failure releases
// partial resources and leaves the bundle empty.
load_assets :: proc(assets: ^Assets, resource_root: string, load_audio := true) -> bool {
	assets^ = {}
	assets.screens.status_bar   = load_resource_texture(resource_root, "screens/status_bar.png")
	assets.screens.game_over    = load_resource_texture(resource_root, "screens/game_over.png")
	assets.screens.you_won      = load_resource_texture(resource_root, "screens/you_won.png")
	assets.screens.branding     = load_resource_texture(resource_root, "intro/00_branding.png")
	for relative_path, image_index in FRONT_END_TEXTURE_PATHS {
		assets.screens.front_end[image_index] =
			load_resource_texture(resource_root, relative_path)
	}

	if load_audio {
		for relative_path, sound_index in BOMB_SOUND_PATHS {
			assets.sounds.bomb[sound_index] =
				load_resource_sound(resource_root, relative_path)
		}
		assets.sounds.item    = load_resource_sound(resource_root, "sounds/item.ogg")
		assets.sounds.hit     = load_resource_sound(resource_root, "sounds/item.ogg")
		assets.sounds.squish  = load_resource_sound(resource_root, "sounds/squish.ogg")
		assets.sounds.ticking = load_resource_sound(resource_root, "sounds/ticking.ogg")
		assets.sounds.menu    = load_resource_sound(resource_root, "sounds/menu.ogg")
		assets.sounds.death   = load_resource_sound(resource_root, "sounds/squish.ogg")

		for relative_path, cue in MUSIC_PATHS {
			assets.music[cue] = load_resource_music(resource_root, relative_path)
		}
	}

	assets.sprites.bomb     = load_resource_texture(resource_root, "sprites/bomb.png")
	assets.sprites.enemy    = load_resource_texture(resource_root, "sprites/enemy.png")
	assets.sprites.objects  = load_resource_texture(resource_root, "sprites/stone.png")
	assets.sprites.player   = load_resource_texture(resource_root, "sprites/player.png")
	assets.sprites.tools    = load_resource_texture(resource_root, "screens/status_tools.png")
	assets.sprites.treasure = load_resource_texture(resource_root, "sprites/treasure.png")
	for relative_path, sprite_index in LEVEL_COMPLETE_SPRITE_PATHS {
		assets.sprites.level_complete[sprite_index] =
			load_resource_texture(resource_root, relative_path)
	}

	assets.effects.explosion =
		load_resource_texture(resource_root, "effects/runtime_explosion.png")
	assets.effects.explosion_smoke =
		load_resource_texture(resource_root, "effects/runtime_explosion_smoke.png")
	assets.effects.flame =
		load_resource_texture(resource_root, "effects/runtime_flame.png")
	assets.effects.smoke[0] =
		load_resource_texture(resource_root, "effects/runtime_smoke_05.png")
	assets.effects.smoke[1] =
		load_resource_texture(resource_root, "effects/runtime_smoke_06.png")

	assets.tiles[.Desert] = load_resource_texture(resource_root, "tiles/desert.png")
	assets.tiles[.Forest] = load_resource_texture(resource_root, "tiles/forest.png")
	assets.tiles[.Lava]   = load_resource_texture(resource_root, "tiles/lava.png")
	assets.tiles[.Winter] = load_resource_texture(resource_root, "tiles/winter.png")

	if !assets_are_valid(assets, load_audio) {
		unload_assets(assets)
		return false
	}
	if load_audio {
		rl.SetSoundPitch(assets.sounds.hit, 0.55)
		// The player-death cue reuses the squish sample at a lower pitch so it
		// reads as heavier than an ordinary enemy kill, without a new asset.
		rl.SetSoundPitch(assets.sounds.death, 0.6)
	}
	return true
}

// assets_are_valid is the single loading boundary for required raylib handles.
assets_are_valid :: proc(assets: ^Assets, require_audio := true) -> bool {
	required_textures := [?]rl.Texture {
		assets.screens.status_bar,
		assets.screens.game_over,
		assets.screens.you_won,
		assets.screens.branding,
		assets.sprites.bomb,
		assets.sprites.enemy,
		assets.sprites.objects,
		assets.sprites.player,
		assets.sprites.tools,
		assets.sprites.treasure,
	}
	for texture in required_textures {
		if !rl.IsTextureValid(texture) do return false
	}
	for texture in assets.screens.front_end {
		if !rl.IsTextureValid(texture) do return false
	}
	for texture in assets.sprites.level_complete {
		if !rl.IsTextureValid(texture) do return false
	}
	required_effects := [?]rl.Texture {
		assets.effects.explosion,
		assets.effects.explosion_smoke,
		assets.effects.flame,
		assets.effects.smoke[0],
		assets.effects.smoke[1],
	}
	for texture in required_effects {
		if !rl.IsTextureValid(texture) do return false
	}
	for texture in assets.tiles {
		if !rl.IsTextureValid(texture) do return false
	}
	if !asset_texture_dimensions_are_valid(assets) do return false

	if require_audio {
		for sound in assets.sounds.bomb {
			if !rl.IsSoundValid(sound) do return false
		}
		required_sounds := [?]rl.Sound {
			assets.sounds.item,
			assets.sounds.hit,
			assets.sounds.squish,
			assets.sounds.ticking,
			assets.sounds.menu,
			assets.sounds.death,
		}
		for sound in required_sounds {
			if !rl.IsSoundValid(sound) do return false
		}
		for music in assets.music {
			if !rl.IsMusicValid(music) do return false
		}
	}

	return true
}

texture_has_dimensions :: proc(texture: rl.Texture, width, height: i32) -> bool {
	return texture.width == width && texture.height == height
}

// asset_texture_dimensions_are_valid protects every grid-packed sprite and
// VFX atlas from silently drawing the wrong cells after an asset replacement.
// Full-screen art is validated as well so old 640x400 files cannot re-enter an
// otherwise complete 800x480 package unnoticed.
asset_texture_dimensions_are_valid :: proc(assets: ^Assets) -> bool {
	if !texture_has_dimensions(assets.screens.status_bar, WINDOW_WIDTH, HUD_STATUS_BAR_HEIGHT) do return false
	if !texture_has_dimensions(assets.screens.game_over, WINDOW_WIDTH, WINDOW_HEIGHT) do return false
	if !texture_has_dimensions(assets.screens.you_won, WINDOW_WIDTH, WINDOW_HEIGHT) do return false
	if !texture_has_dimensions(assets.screens.branding, WINDOW_WIDTH, WINDOW_HEIGHT) do return false
	for texture in assets.screens.front_end {
		if !texture_has_dimensions(texture, WINDOW_WIDTH, WINDOW_HEIGHT) do return false
	}

	if !texture_has_dimensions(assets.sprites.bomb, MAP_TILE_SIZE, BOMB_SPRITE_COUNT * MAP_TILE_SIZE) do return false
	if !texture_has_dimensions(assets.sprites.enemy, MAP_TILE_SIZE, ENEMY_SPRITE_COUNT * MAP_TILE_SIZE) do return false
	if !texture_has_dimensions(assets.sprites.objects, 4 * MAP_TILE_SIZE, 4 * MAP_TILE_SIZE) do return false
	if !texture_has_dimensions(assets.sprites.player, PLAYER_SPRITE_SIZE, PLAYER_SPRITE_COUNT * PLAYER_SPRITE_SIZE) do return false
	if !texture_has_dimensions(assets.sprites.tools, 2 * MAP_TILE_SIZE, 2 * MAP_TILE_SIZE) do return false
	if !texture_has_dimensions(assets.sprites.treasure, MAP_TILE_SIZE, TREASURE_SPRITE_COUNT * MAP_TILE_SIZE) do return false
	for texture in assets.sprites.level_complete {
		if !texture_has_dimensions(texture, LEVEL_COMPLETE_SPRITE_WIDTH, LEVEL_COMPLETE_SPRITE_HEIGHT) do return false
	}

	if !texture_has_dimensions(assets.effects.explosion, EFFECT_EXPLOSION_ATLAS_WIDTH, EFFECT_EXPLOSION_ATLAS_HEIGHT) do return false
	if !texture_has_dimensions(assets.effects.explosion_smoke, EFFECT_EXPLOSION_ATLAS_WIDTH, EFFECT_EXPLOSION_ATLAS_HEIGHT) do return false
	if !texture_has_dimensions(assets.effects.flame, EFFECT_FLAME_ATLAS_WIDTH, EFFECT_FLAME_ATLAS_HEIGHT) do return false
	for texture in assets.effects.smoke {
		if !texture_has_dimensions(texture, EFFECT_SMOKE_TEXTURE_SIZE, EFFECT_SMOKE_TEXTURE_SIZE) do return false
	}
	for texture in assets.tiles {
		if !texture_has_dimensions(texture, 8 * MAP_TILE_SIZE, 8 * MAP_TILE_SIZE) do return false
	}
	return true
}

// unload_assets releases every application-owned raylib resource during
// shutdown or failed startup, then clears the handles to make cleanup idempotent.
unload_assets :: proc(assets: ^Assets) {
	unload_texture(assets.screens.status_bar)
	unload_texture(assets.screens.game_over)
	unload_texture(assets.screens.you_won)
	unload_texture(assets.screens.branding)
	for texture in assets.screens.front_end {
		unload_texture(texture)
	}

	for sound in assets.sounds.bomb {
		unload_sound(sound)
	}
	unload_sound(assets.sounds.item)
	unload_sound(assets.sounds.hit)
	unload_sound(assets.sounds.squish)
	unload_sound(assets.sounds.ticking)
	unload_sound(assets.sounds.menu)
	unload_sound(assets.sounds.death)
	for music in assets.music {
		unload_music(music)
	}

	unload_texture(assets.sprites.bomb)
	unload_texture(assets.sprites.enemy)
	unload_texture(assets.sprites.objects)
	unload_texture(assets.sprites.player)
	unload_texture(assets.sprites.tools)
	unload_texture(assets.sprites.treasure)
	for texture in assets.sprites.level_complete {
		unload_texture(texture)
	}

	unload_texture(assets.effects.explosion)
	unload_texture(assets.effects.explosion_smoke)
	unload_texture(assets.effects.flame)
	for texture in assets.effects.smoke {
		unload_texture(texture)
	}

	for tile in assets.tiles {
		unload_texture(tile)
	}

	// Clear resource handles so ownership has visibly ended and an accidental
	// second cleanup remains harmless.
	assets^ = {}
}

// unload_texture safely releases one optional texture while walking the asset
// bundle during cleanup.
unload_texture :: proc(texture: rl.Texture) {
	if rl.IsTextureValid(texture) do rl.UnloadTexture(texture)
}

// unload_sound safely releases one optional sound; invalid handles occur when
// audio is unavailable or startup stops after a partial load.
unload_sound :: proc(sound: rl.Sound) {
	if rl.IsSoundValid(sound) do rl.UnloadSound(sound)
}

// unload_music safely releases one optional streaming track.
unload_music :: proc(music: rl.Music) {
	if rl.IsMusicValid(music) do rl.UnloadMusicStream(music)
}

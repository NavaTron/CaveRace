...
odin build . $BUILD_FLAGS -vet -vet-cast -vet-style -vet-tabs -warnings-as-errors \
	-out:"../dist/macos/$APP_NAME.app/Contents/MacOS/CaveRace"
...
cp "$PROJECT_DIRECTORY/packaging/macos/PrivacyInfo.xcprivacy" "$CONTENTS_DIRECTORY/Resources/PrivacyInfo.xcprivacy"
...
find "$CONTENTS_DIRECTORY/Resources/media" -type f \( -name ".DS_Store" -o -name "Thumbs.db" \) -delete

test -x "$CONTENTS_DIRECTORY/MacOS/CaveRace"
test -f "$CONTENTS_DIRECTORY/Resources/CaveRace.icns"
test -f "$CONTENTS_DIRECTORY/Resources/PrivacyInfo.xcprivacy"
test -f "$CONTENTS_DIRECTORY/Resources/media/screens/status_bar.png"
...
test -f "$CONTENTS_DIRECTORY/Resources/levels/end.bin"
...
if [ -n "$NOTARY_PROFILE" ]; then
	...
	NOTARY_ARCHIVE="$DIST_DIRECTORY/$APP_NAME-notarize.zip"
	rm -f "$NOTARY_ARCHIVE"
